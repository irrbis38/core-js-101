...
function findFirstSingleChar(str) {
  ...
  return arrFromStr.filter((item) => arrFromStr.indexOf(item) === arrFromStr.lastIndexOf(item))[0];
}
...
function getDigitalRoot(num) {
  ...
  const sum = numArr.reduce((total, el) => total + el, 0);
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < brackets.length; i += 1) {
    while (res) {
      if (res.includes(brackets[i])) {
        res = res.replace(brackets[i], '');
        i = 0;
      } else {
        break;
      }
